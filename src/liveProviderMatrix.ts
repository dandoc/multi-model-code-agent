...
const SMOKE_MESSAGES: ChatMessage[] = [{ role: 'user', content: 'Reply with exactly OK.' }];
...
export async function runLiveProviderSmoke(
  ...
  for (const entry of diagnostics) {
    ...
    const startedAt = now();
    try {
      const reply = await adapter.complete(SMOKE_MESSAGES, nextConfig);
      ...
      results.push({
        ...
        currentModel: entry.currentModel,
        baseUrl: entry.baseUrl,
        status: preview ? 'passed' : 'failed',
        elapsedMs,
        summary: preview
          ? 'Live completion request succeeded.'
          : 'The provider returned an empty assistant reply.',
        detailLines: preview ? [] : ['The smoke prompt completed but the assistant reply was empty.'],
  ...
  return results
    .map((result) => {
      ...
      if (result.replyPreview) {
        lines.push(`reply        ${result.replyPreview}`);
      }
